...
export const StudentDashboard: React.FC = () => {
  ...
  const recentActivities = registrations
    ...
    .map(reg => {
      ...
      return {
        title: `${reg.status === 'approved' ? 'Registered for' : 'Registration pending for'} ${hackathon?.title || reg.hackathonId}`,
        time: formatDate(reg.submittedAt),
        type: 'registration'
      };
    });
            ...
            <div className="space-y-3">
              {upcomingEvents.map((event, index) => (
                <div key={index} className="border-l-4 border-blue-500 pl-4 py-2">
                  ...
                  <span className={`inline-block mt-2 px-2 py-1 text-xs rounded-full ${event.status === 'registered'
